...
class TestDefaultChangeMigration:
    def test_full_era_no_suffix_output_regenerates_at_new_default(
        ...
        after the default flips to HIGH — the version gate forces
        regeneration even when the source is untouched, so the version bump
        shipping this change re-renders every no-suffix file at the new
        default.

        The gate on this single-file path is the format-renderer override
        ``HtmlRenderer.is_outdated`` (html/renderer.py) — which sniffs the
        HTML's embedded ``<!-- Generated by … vX -->`` — NOT the base
        ``Renderer.is_outdated`` (returns None) nor the ``is_html_outdated``
        module function. Isolates it: the output is left NEWER than the
        source (source_is_newer is False), so only the version sniff can
        trigger regen.
        """
        src = _write(
            tmp_path,
