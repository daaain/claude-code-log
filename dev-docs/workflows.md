...
- **Side-channel grafting** (`_graft_agent_sidechannel`): each agent's
  ...
  space. The side-channel renders at HOOK depth regardless of the main
...
`generate_template_messages(entries)` call at **HOOK depth** (everything)
regardless of the main render's depth — at `--depth tool` an agent's
